...
namespace TestingSamples.FilesDiffControls
{
    ...
    internal static class Helper
    {
        ...
        /// </summary>
        internal static void RenderInlineDiffs(InternalLinesViewerView viewer, ICollection<DiffPiece> lines, UIElement source, int contextLineCount)
        {
            viewer.Clear();
        ...
        internal static void InsertLines(InternalLinesViewerView panel, List<DiffPiece> lines, bool isOld, UIElement source, int contextLineCount)
        {
        ...
        private static async Task InsertLinesAsync(Guid guid, InternalLinesViewerView panel, List<DiffPiece> lines, bool isOld, UIElement source, int contextLineCount)
        {   // For performance.
            if (lines == null || panel == null) return;
        ...
        internal static void CollapseUnchangedSections(InternalLinesViewerView panel, int contextLineCount)
        {
            var i = -1;
            var was = false;
            var last = 0;
        ...
        internal static void ExpandUnchangedSections(InternalLinesViewerView panel)
        {
        ...
        internal static bool GoTo(InternalLinesViewerView panel, int lineIndex)
        {
            try
            {
        ...
        /// <returns>true if it has turned to the specific line; otherwise, false.</returns>
        internal static bool GoTo(InternalLinesViewerView panel, DiffPiece line)
        {
        ...
        internal static DiffPiece GetLine(InternalLinesViewerView panel, int lineIndex)
        {
            foreach (var item in panel.ValuePanel.Children)
            {
                if (item is not FrameworkElement ele || ele.Tag is not DiffPiece line || line?.Position != lineIndex) continue;
        ...
        internal static IEnumerable<DiffPiece> GetLinesInViewport(InternalLinesViewerView panel, VisibilityLevels level)
        {
        ...
        /// <param name="level">The optional visibility level.</param>
        /// <returns>All lines.</returns>
        internal static IEnumerable<DiffPiece> GetLinesBeforeViewport(InternalLinesViewerView panel, VisibilityLevels level)
        {
            var states = GetLineViewportStates(panel, level);
            foreach (var item in states)
            {
        ...
        /// <param name="level">The optional visibility level.</param>
        /// <returns>All lines.</returns>
        internal static IEnumerable<DiffPiece> GetLinesAfterViewport(InternalLinesViewerView panel, VisibilityLevels level)
        {
            var states = GetLineViewportStates(panel, level);
        ...
        internal static ContextMenu CreateLineContextMenu(FrameworkElement parentElement)
        {
            ...
            {
                Header = GetButtonName("Copy this line", "C")
            };
        ...
        /// <param name="level">The optional visibility level.</param>
        /// <returns>All lines.</returns>
        private static IEnumerable<Tuple<DiffPiece, bool>> GetLineViewportStates(InternalLinesViewerView panel, VisibilityLevels level)
        {
            var scrollView = panel.ValueScrollViewer;
            var point = new Point(0, 0);
            switch (level)
        ...
        private static void InsertLinesInteral(InternalLinesViewerView panel, List<DiffPiece> lines, bool isOld, UIElement source, bool disableSubPieces = false)
        {
            foreach (var line in lines)
            {
                if (line == null)
